    ...
    async execute(oldMember, newMember) {
        try {
            if (oldMember.communicationDisabledUntilTimestamp !== newMember.communicationDisabledUntilTimestamp) {
                ...
                const logEmbed = new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: 'Fin du timeout', value: newMember.communicationDisabledUntil ? `<t:${Math.floor(newMember.communicationDisabledUntilTimestamp / 1000)}:F>` : 'Aucun' }
                    )
                    .setTimestamp()
                    .setFooter({ text: 'Magearna', iconURL: newMember.client.user.displayAvatarURL() });

                const logChannel = newMember.client.channels.cache.get(process.env.LOGCHANNEL);
            ...
            if (addedRoles.size > 0) {
                changes.push({ name: 'Rôles ajoutés', value: addedRoles.map(role => role.name).join(', ') });
            }
            if (removedRoles.size > 0) {
                changes.push({ name: 'Rôles retirés', value: removedRoles.map(role => role.name).join(', ') });
            }
