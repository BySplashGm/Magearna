    ...
    async execute(oldChannel, newChannel) {
        try {
            ...
            if (oldChannel.type !== newChannel.type) {
                ...
                changes.push({ name: 'Type', value: `\`${typeToString(oldChannel.type)}\` → \`${typeToString(newChannel.type)}\`` });
            }
            if (oldChannel.topic !== newChannel.topic) {
                changes.push({ name: 'Sujet', value: `\`${oldChannel.topic || "Aucun"}\` → \`${newChannel.topic || "Aucun"}\`` });
            }
            ...
            const logEmbed = new EmbedBuilder()
                ...
                .setTimestamp()
                .setFooter({ text: 'Magearna', iconURL: newChannel.client.user.displayAvatarURL() });

            const logChannel = newChannel.client.channels.cache.get(process.env.LOGCHANNEL);
